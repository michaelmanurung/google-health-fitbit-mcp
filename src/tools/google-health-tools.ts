...
// One client per config keeps the token store and lazy SQLite cache handle alive across tool
// calls instead of re-opening them on every request. getConfig() is still evaluated per call so
// env/local-config changes are picked up (the key changes and a fresh client replaces the old one).
let cachedClient: { key: string; instance: GoogleHealthClient } | undefined;

function client(): GoogleHealthClient {
  const config = getConfig();
  const key = JSON.stringify(config);
  if (cachedClient?.key !== key) cachedClient = { key, instance: new GoogleHealthClient(config) };
  return cachedClient.instance;
}

function endpointOutput(endpoint: string, privacy_mode: PrivacyMode, data: unknown) {
  return { endpoint, privacy_mode, data };
}

// Shared body for every raw-endpoint read tool: resolve privacy mode, call Google, apply the
// privacy filter, and fall back to a redacted endpoint error. Keeps each tool registration down
// to its endpoint, fetcher and markdown rendering.
async function runEndpointTool(
  endpoint: string,
  params: { privacy_mode?: PrivacyMode; response_format: ResponseFormat },
  fetch: (api: GoogleHealthClient) => Promise<unknown>,
  markdown: (data: unknown) => string
) {
  let mode: PrivacyMode = params.privacy_mode ?? "structured";
  try {
    mode = resolvePrivacyMode(getConfig(), params.privacy_mode);
    const data = applyPrivacy(endpoint, await fetch(client()), mode);
    return makeResponse(endpointOutput(endpoint, mode, data), params.response_format, markdown(data));
  } catch (error) {
...
export function registerGoogleHealthTools(server: McpServer): void {
  ...
  server.registerTool("google_health_quickstart", {
    ...
    const payload = {
      ...
      migration_note: "Fitbit accounts are migrating to Google Health Connect. If you previously used fitbit-mcp-unofficial and now own a Pixel Watch (or installed Google Health Connect on Android), google-health-fitbit-mcp is the forward-looking connector — your tokens are different but the data domains overlap.",
  ...
  }, async (params) => runEndpointTool(
    "/v4/users/me/identity",
    params,
    (api) => api.getIdentity(),
    (data) => bulletList("Google Health Identity", data as Record<string, unknown>)
  ));
  ...
  server.registerTool("google_health_get_profile", {
    ...
    params,
    (api) => api.getProfile(),
    (data) => bulletList("Google Health Profile", data as Record<string, unknown>)
  ));
  ...
  server.registerTool("google_health_get_settings", {
    ...
    params,
    (api) => api.getSettings(),
    (data) => bulletList("Google Health Settings", data as Record<string, unknown>)
  ));

  server.registerTool("google_health_list_paired_devices", {
    title: "List Paired Devices",
    description: "List the Fitbit trackers, Pixel Watches and other devices paired to the authenticated Google Health account. Useful to answer 'is my watch syncing?' and to see which device produces which data. Read-only.",
    inputSchema: PairedDevicesInputSchema.shape,
    outputSchema: EndpointDataOutputSchema.shape,
    annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: true }
  }, async (params) => runEndpointTool(
    "/v4/users/me/pairedDevices",
    params,
    (api) => api.listPairedDevices({ pageSize: params.page_size, pageToken: params.page_token }),
    (data) => formatDataPointsMarkdown("Google Health Paired Devices", { endpoint: "/v4/users/me/pairedDevices" }, data)
  ));

  server.registerTool("google_health_get_irn_profile", {
    title: "Get Irregular Rhythm Notification Profile",
    description: "Get the user's irregular rhythm notification (IRN) engagement status — whether the AFib detection feature is enabled on their Fitbit/Pixel Watch. Requires the googlehealth.irn.readonly scope (setup --scope-preset heart). Read-only, non-diagnostic.",
    inputSchema: SimpleReadInputSchema.shape,
    outputSchema: EndpointDataOutputSchema.shape,
    annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: true }
  }, async (params) => runEndpointTool(
    "/v4/users/me/irnProfile",
    params,
    (api) => api.getIrnProfile(),
    (data) => bulletList("Google Health IRN Profile", data as Record<string, unknown>)
  ));

  server.registerTool("google_health_get_data_point", {
    title: "Get Google Health Data Point",
    description: "Fetch one data point by id — e.g. a specific sleep session, exercise or weight entry found via list/reconcile. Accepts the bare id or the full resource name from a previous response. Only data types whose official_operations include 'get' support this (see google_health_list_data_types).",
    inputSchema: GetDataPointInputSchema.shape,
    outputSchema: EndpointDataOutputSchema.shape,
    annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: true }
  }, async (params) => {
    const endpoint = `/v4/users/me/dataTypes/${params.data_type}/dataPoints/{id}`;
    return runEndpointTool(
      endpoint,
      params,
      (api) => api.getDataPoint(params.data_type, params.data_point_id),
      (data) => formatDataPointsMarkdown("Google Health Data Point", { endpoint, data_type: params.data_type }, data)
    );
  });
  ...
  server.registerTool("google_health_list_data_points", {
    ...
    return runEndpointTool(
      endpoint,
      params,
      (api) => api.listDataPoints({
        dataType: params.data_type,
        filter: params.filter,
        pageSize: params.page_size,
        pageToken: params.page_token
      }),
      (data) => formatDataPointsMarkdown("Google Health Data Points", { endpoint, data_type: params.data_type }, data)
    );
  });
  ...
  server.registerTool("google_health_reconcile_data_points", {
    ...
    return runEndpointTool(
      ...
      (data) => formatDataPointsMarkdown("Google Health Reconciled Data", { endpoint, data_type: params.data_type, data_source_family: params.data_source_family ?? "all" }, data)
    );
  });
  ...
  server.registerTool("google_health_daily_rollup", {
    ...
    return runEndpointTool(
      ...
      (data) => formatDataPointsMarkdown("Google Health Daily Rollup", { endpoint, data_type: params.data_type, data_source_family: params.data_source_family }, data)
    );
  });
  ...
  server.registerTool("google_health_rollup", {
    ...
    return runEndpointTool(
      ...
      (data) => formatDataPointsMarkdown("Google Health Rollup", { endpoint, data_type: params.data_type, data_source_family: params.data_source_family }, data)
    );
  });
      ...
      try {
        if (explicit_user_intent !== true) {
          return makeResponse(
            ...
            response_format,
            bulletList("Wellness Profile Update", {
              ok: false,
              error: "USER_ACTION_REQUIRED",
      ...
      try {
        ...
        const payload = {
          ...
          cross_connector_hint:
            "This profile is shared across all Wellness connectors. Answering once populates context for whoop, garmin, oura, fitbit, strava, polar, withings, apple-health, samsung-health, google-health, nourish, cycle-coach, cgm, and air."
        };
        return makeResponse(payload, response_format, bulletList("Wellness Onboarding", {
          locale: flow.locale,
