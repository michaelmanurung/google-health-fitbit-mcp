...
`google_health_revoke_access` and its client revocation/token-deletion implementation have been removed. MCP callers cannot invoke this operation through either transport; there is no alias or opt-in switch. Grant revocation and local credential deletion must happen outside MCP. Authorization URL generation, local CLI authorization, automatic reauthorization, and token refresh remain available. MCP code exchange has also been removed as described below. Restart the rebuilt server and refresh the harness tool catalog after upgrading. The original review line references below may have shifted after this change.

## Code exchange remediation

`google_health_exchange_code` has been removed from MCP registration, schemas, and the agent manifest. Neither stdio nor authenticated HTTP callers can exchange a supplied code or replace credentials through this tool, even with `explicit_user_intent=true`. Run `google-health-fitbit-mcp-server auth` in a local terminal to connect or change accounts; its loopback callback validates state before exchanging the code. The client exchange method remains internal for CLI authorization and automatic reauthorization. This closes the direct MCP code-exchange path; it does not add a universal confirmation gate to automatic reauthorization. Regression coverage in `scripts/auth-boundary-test.mjs` checks both transports with dummy credentials, blocked outbound fetches, and unchanged token-file assertions. Rebuild, restart, and refresh the harness tool catalog after upgrading.
...
| MCP surface | `src/tools/google-health-tools.ts` registers 27 tools; `src/resources/google-health-resources.ts` registers seven resources; `src/prompts/google-health-prompts.ts` registers three workflow prompts. `src/schemas/common.ts` defines Zod contracts. |
...
| OAuth and account mutation | `google_health_get_auth_url` previews an authorization URL. Code exchange, revocation, and credential deletion are not exposed through MCP. |
...
| **Medium** | OAuth URL preview accepts caller-chosen scopes and optional state, but MCP can no longer exchange the resulting code. CLI and automatic loopback flows validate state but generate only four random bytes (`src/services/interactive-auth.ts:111-124`, `src/services/oauth-loopback.ts:37-59`). | The manual MCP exchange path is closed. Keep authorization a human-initiated local operation; longer server-generated state remains recommended for the loopback flows. |
...
For a first trial, run the reviewed local build over stdio under a dedicated local user, with the smallest OAuth scopes needed. Inspect `~/.google-health-mcp/config.json` and the generated harness command before entering credentials. Allowlist read tools and omit `profile_update` until its caller and confirmation policy is settled. Code exchange is unavailable through MCP; use the local `auth` CLI. Do not enable HTTP on a network interface without authentication. Keep SQLite disabled unless its persistence is required; protect or remove old cache files when changing accounts. These steps reduce exposure but do not independently verify the repository, npm artifact, dependencies, or Google's live API behavior.
