...
| --- | --- | --- |
| **Critical if network-exposed** | HTTP `POST /mcp` has no application authentication or caller authorization (`src/index.ts:31-64`). CORS limits cooperative browsers only; it does not authenticate curl, other processes, or a network client. Host is configurable beyond the loopback default. | A reachable caller can invoke health-data and account tools using the server's stored tokens. Keep stdio/loopback only, or add strong authentication and network isolation before any remote deployment. |
